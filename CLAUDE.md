...
Primary dev environment is the `.devcontainer/` (VS Code Dev Containers, workspace at `/workspace/opus`), provisioning Node 24, pnpm, AWS CLI, Docker-outside-of-docker, gh, terraform, python. Host/container setup runs via `.devcontainer/scripts/` (`initialize.sh`, `post-create`, `post-attach`).
