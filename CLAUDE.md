...
- **CI/CD** is GitHub Actions (`.github/workflows/`): `ci.yaml` runs build→lint→test on pushes to `main` and on PRs; on `main` it then calls `publish.yaml` and `deploy.yaml` as reusable workflows. PRs against `main` get ephemeral **preview** stages (`pr-<number>`), torn down by `destroy-preview.yml`; `merge-checks.yaml` enforces that the README and repo-kit config are in sync. Deploys assume AWS via OIDC. Full details: `docs/CICD.md`.
...
## Workspace credentials (devcontainer)

In the development workspace, agents hold only **short-lived (≤1h), pre-scoped credentials**, vended by a privileged `admin-sidecar` container onto a read-only shelf at `/creds`. Nothing to set up or mint:

- **GitHub**: `gh` and `git` (HTTPS) authenticate automatically — the `gh` wrapper serves an App installation token scoped to this repo from the shelf.
- **AWS**: ambient credentials come from `AWS_SHARED_CREDENTIALS_FILE=/creds/aws/credentials`; `[default]` is the `developer-ai-agent` role. Any other vended roles are named sections — select with `AWS_PROFILE=<name>`.
- **When auth fails** (`ExpiredToken` from AWS, or `gh`/`git` suddenly unauthenticated plus a stderr breadcrumb): vending has stalled, usually because the human's SSO session expired. Check `cat /creds/status/*`. The fix is human-only (`docker exec -it admin-sidecar aws-refresh-sso <profile>` from a host terminal; fresh creds appear ≤60s later). Don't try to work around missing credentials — surface it and wait.
- Scope changes (more repos, roles, permissions) happen in the sidecar's config, never from here. Full design: `/workspace/.devcontainer/README.md` (outside this repo).
