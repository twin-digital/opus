...
**First draft, in progress.** The player-facing interface for Farwatch, scoped for now to **the cold
open through the start of turn 1** — meeting the compact you've inherited and sending its first
expedition. Leans on the vocabulary in [glossary.md](glossary.md); terms still settling on a name are
marked **(working name)**, the same convention the glossary uses.

Audience: whoever builds the player web app — a *separate* surface from the dev `inspector` (which
exposes seeds, prompts, and the resolver's guts; the player never sees any of it).
...
> **Look & feel is deliberately deferred** to a visual pass — see the parked draft at the bottom. We
> define the surfaces here first, then render these same surfaces in 2–3 candidate moods and choose
> from real comparisons.
...
**summoned** (never parked — see [The frame](#the-frame--attention-not-navigation)).
...
**turns its regard**. Navigation is therefore **attention, not space**: the thing attended to coalesces
into the light and everything else recedes into the dark. **One thing is ever in focus** — the letter you
hold *is* the screen. There is **no nav rail and no parked chrome**: a persistent strip is the one thing
that cannot coalesce, so it would read as exactly the HUD the
[fog model](#truth-and-your-picture-of-it) swears off — persistence is itself a claim of machine-truth.

**The disorientation is the patron's untrained attention — and it *is* the navigation curve.** A
newly-Called regard can barely hold focus; it is *swept*. Learning to focus the regard is the progression:

- **Carried (Alpha-0).** Just-Called and disoriented, you attend only to what is set before you, in sequence.
  The **procession** carries you, and its control is the **page-turn**: finish the founding prospect and
  the Board rises; choose, and the Roster; assemble, and the Dispatch; **seal**, and you fade. You never
  pick a room — you turn pages and the corridor unfolds. *No free surface-movement in Alpha-0, and that is
  diegetic (you cannot yet focus), not a gap.*
- **Summon (later).** With practice you call a thing into focus out of sequence; the prior recedes (set
  down, not destroyed). Still one-in-focus.
- **Free movement (later still).** The regard learns to direct itself — Board ↔ Roster at will. Still no
  parked strip; movement is always attention shifting, never traversing a space. The strip never returns.

**Facts are summoned, not parked — the writing is the index.** A fact surfaces two ways, neither a
toolbar:
...
- **Fallback — one steward-held door.** The prose can't always name a fact (a quiet tiding never mentions
  ...
  [**steward**](#the-steward--your-voice-on-the-ground) — the single in-world presence allowed to hold
  "what's always true." Reach for the steward (their mark, or one thin marginal gutter beside the sheet)
  and the facts rise as **named artifacts** — the almanac, the ledger, the writ — each opened and let
  lie, each in its keeper's hand with the fog intact. *Not* three corner glyphs: a glyph-row is a toolbar
  (it scans as chrome, needs a legend, floats with no speaker, and regrows a rail when Standing/Bonds add
  a fourth). One door scales by putting another artifact *behind* it.

A summoned fact **rises as a page and recedes** — a book you open, not a readout that stares at you. (Its
label is the **fact-name**, rendered as its artifact: Charter → a sealed writ, Ledger → a ledger, Season →
an almanac.) This makes "authored artifact, never HUD" literal: a fact you *call up* can be fallible in a
way a parked value cannot.

This is consistent with the [navigation curve](#the-frame--attention-not-navigation): routing facts
through the prose and the steward is **not** the regard summoning at will — the writing and the steward do
the holding (the steward is the prosthetic for the unfocused regard). Free, self-directed summoning is the
learned, later capability.
...
**Two controls only** — the **page-turn** (advance the procession, reveal the next beat) and the **seal**
(commit, and fade). Everything else is reading.

The surfaces still exist — **Missives · Board · Roster · Counsel** *(deferred)* **· Dispatch** — but in
Alpha-0 they are **stations of the procession**, reached by turning pages, not rooms in a menu.

```
        +----------------------------+
        |  "...word came back."      |   the steward, then --
        |                            |
        |   the letter you hold      |   one centered column;
        |   -- the chronicle --      |   the reading is the screen
        |                            |
        |   a cost ticks -> the      |   facts rise inline,
        |   ledger rises, recedes    |   then recede (never parked)
        |                            |
        |        turn the page >     |   one of two controls
        +----------------------------+   (the other: the seal)
```

(Single-column expression in the `reading-first` [mockups](mockups/); the earlier nav-rail layout,
`ui-heavy`, is superseded.)
...
goals*. That permanence-of-knowledge model lives in [design-deep §7](design-deep.md); it is **out of
...
- **The steward closest, the regard least focused** — your attention at its most untrained, so the
  ...
  curve](#the-frame--attention-not-navigation)). Most guidance here; fog at zero; edicts inert.
...
- **The Desk** — the frame; **navigation is attention** (the procession and its page-turn; one thing in
  focus, the rest receded; Dispatch set apart as the sealing act).
...
**Alpha-1 … Alpha-N — iteration headroom.** Refinements discovered by playing the alpha land here, each its
own numbered cut. Nothing is pre-assigned; this band exists so pre-MVP polish isn't mislabelled as MVP.
...
Captured from discussion; **not settled** — to be decided by rendering the surfaces above in candidate
moods. The brief so far:
