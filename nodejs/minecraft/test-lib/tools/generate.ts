...
/**
 * A dependency's declared version. The manifest is read by walking up from the resolved entry
 * point rather than resolving `<specifier>/package.json`, which an `exports` map need not expose.
 */
const versionOf = (specifier: string): string => {
  let directory = path.dirname(require.resolve(specifier, { paths: [packageRoot] }))
  for (;;) {
    ...
    if (parent === directory) {
      return fail(`${specifier} resolves to no package manifest`)
    }
    directory = parent
  }
}
...
const vanillaDataVersion: string = versionOf('@minecraft/vanilla-data')

/** The provenance line every emitted file opens with, naming the source it was derived from. */
const headerFrom = (source: string, what: string): string =>
  [`// GENERATED by tools/generate.ts from ${source}. Do not edit.`, `// ${what}`, ''].join('\n')

const header = (what: string): string => headerFrom(`@minecraft/server ${serverVersion}`, what)
...
// development dependency: nothing a consumer installs pulls it in. Committed, so a content bump
// reads as a diff of which ids moved; the counts asserted in presets.test.ts are what fail first.
const vanillaPath = path.join(outDir, 'vanilla.ts')
const vanillaConfig = await resolveConfig(vanillaPath)
fs.writeFileSync(
  vanillaPath,
  await format(
    [
      headerFrom(
        `@minecraft/vanilla-data ${vanillaDataVersion}`,
        'The vanilla id lists a preset registers, as that source carries them.',
      ),
    ...
    { ...vanillaConfig, filepath: vanillaPath },
  ),
)
