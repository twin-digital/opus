# Lock Link: Automatic Door-Code Delivery — Proposal
...
delivery fails, the guest calls, and staff drop what they're doing to sort it out (or the guest
is forced to cancel/delay their check-in if staff is unavailable).
...
- The original smart-lock platform (Jervis, driving Tuya locks) had persistent code-provisioning
  failures it was unable to resolve. Bonded Locks's search for a replacement found exactly one
...
- A guest who books weeks ahead gets their code the day before arrival. A guest who books from
  the parking lot gets one in roughly 15 minutes. The experience is smooth regardless of when 
  the reservation was made: days/weeks in advance, the same day, or even after the check-in time
- No guest is told "your code is on the way" or receives an empty email with missing codes
- Temporary failures in lock connectivity or the Lynx platform do not prevent a guest from getting
  access to the room
