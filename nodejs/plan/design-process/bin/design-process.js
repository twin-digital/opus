...
// condition; map the repo's NodeNext `.js` specifiers to their `.ts` source). Installed as a published package,
// `src` is absent (the `files` allowlist ships `dist` only), so we run the compiled bin instead.
