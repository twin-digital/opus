...
describe('LaunchpadController keyboard routing', () => {
  ...
  it('routes every note to every channel by default', () => {
    ...
    deviceHandlers.get('noteon')?.({ channel: 0, note: 40, velocity: 64 })
  ...
  it('routes notes by key range once routes are set', () => {
    ...
    deviceHandlers.get('noteon')?.({ channel: 0, note: 59, velocity: 64 })
    deviceHandlers.get('noteon')?.({ channel: 0, note: 60, velocity: 64 })
  ...
  it('matches the whole keyboard when a route has no range', () => {
    ...
    deviceHandlers.get('noteon')?.({ channel: 0, note: 0, velocity: 64 })
    deviceHandlers.get('noteon')?.({ channel: 0, note: 127, velocity: 64 })
  ...
  it('plays a note at most once per channel when overlapping routes hit the same channel', () => {
    ...
    deviceHandlers.get('noteon')?.({ channel: 0, note: 62, velocity: 64 })
  ...
  it('routes note-offs through the same table', () => {
    ...
    deviceHandlers.get('noteoff')?.({ channel: 0, note: 59, velocity: 0 })
  ...
  it('still honors mute on a routed channel, except for velocity-0 releases', () => {
    ...
    deviceHandlers.get('noteon')?.({ channel: 0, note: 40, velocity: 0 })
