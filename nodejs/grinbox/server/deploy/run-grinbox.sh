...
#
# The Daemon reads everything from
# process.env (src/config.ts). The sealed bundle is a plain
# `KEY=VALUE`-per-line env file, produced and sealed at provision time by the
# grinbox Ansible role (d-9hyzpk7j: encryption over a key grinbox is handed).
#
...
exec /usr/bin/node /opt/grinbox/server/main.js
