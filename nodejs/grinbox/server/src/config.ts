...
const keySchema = z.string({ error: `${TOKEN_ENC_KEY_ENV} is required` }).transform((raw, ctx) => {
...
const configSchema = z.object({
   ...
   * window selection, per-section rendering, any LLM prose blocks, and the
   * send, so its budget is deliberately larger than the per-Operator
   * `operatorTimeoutMs`. On expiry the run's in-flight calls abort and the
   * run is recorded `failed`.
   */
  digestTimeoutMs: z.coerce.number().int().positive().default(120_000),
