...
function mockClient(overrides: Partial<GmailProviderClient> = {}): GmailProviderClient {
  return {
    ...
    listHistory: vi.fn(async (): Promise<GmailHistoryPage> => ({
      addedMessageIds: [],
      historyId: 'H0',
    })),
    getMessage: vi.fn(async (id: string): Promise<GmailMessagePayload> => ({
      id,
      threadId: null,
      snippet: null,
      internalDate: null,
...
describe('GmailProvider.listCandidates', () => {
  ...
  it('incremental with zero new messages: cursor still advances to the new historyId (no re-scan-forever)', async () => {
    ...
    const listHistory = vi.fn(async (): Promise<GmailHistoryPage> => ({
      addedMessageIds: [],
