 ...
 *
 * Drives **Apply Category**, **Archive**, and **Notify** runs end-to-end
 * through `runWorker` over the real per-run
 * {@link buildMakeUnderlyingClients} builder, asserting:
 ...
 *  - archive resolves the same Account credential and calls `messages.modify`
...
describe('credential-backed Action clients through the worker', () => {
  ...
  async function claimRun(
    typeKey: 'apply_category' | 'archive' | 'notify',
    configJson: string,
  ...
  it('apply_category fires against the Account gmail credential (name→id resolve + modify)', async () => {
    ...
    // resource_op_succeeded event + usage landed for apply_category.
  ...
  it('archive fires against the Account gmail credential (removes INBOX)', async () => {
    const seed = await seedBase(db)
    await seedDefaultLimits(db, seed.userId)
    await seedGmailCredential(db, seed.userId, seed.accountId)
    ...
    const row = await claimRun('archive', configJson, seed)
    ...
    // No label resolution — archive only removes the INBOX membership.
    ...
    // resource_op_succeeded event + usage landed for archive.
    const events = await db
      .selectFrom('triage_events')
      .selectAll()
      .where('triage_id', '=', row.triage_id)
  ...
  it('unconfigured daemon (no OAuth client) → mailbox apply_category is "not configured"', async () => {
    const seed = await seedBase(db)
    await seedDefaultLimits(db, seed.userId)
    await seedGmailCredential(db, seed.userId, seed.accountId)
    ...
    await expect(
      clients.mailbox.apply_category({ backendMessageId: 'm1', category: 'L' }, new AbortController().signal),
