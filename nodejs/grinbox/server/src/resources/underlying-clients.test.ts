...
describe('buildUnderlyingClients gating', () => {
  ...
  it('always leaves the mail/pushover clients "not configured" (M2)', () => {
    const clients = buildUnderlyingClients(testConfig({ bedrockRegion: 'us-east-1' }))
    const sig = new AbortController().signal
    expect(() => clients.mailbox.apply_category({ backendMessageId: 'x', category: 'L' }, sig)).toThrow(
      /no Resource client is configured/i,
    )
    expect(() => clients.mailbox.archive({ backendMessageId: 'x' }, sig)).toThrow(/no Resource client is configured/i)
    expect(() => clients.mail_sender.send_message({ to: 'user@example.com', subject: 's', body: 'b' }, sig)).toThrow(
      /no Resource client is configured/i,
    )
