...
describe('checkAndConsumeLimits', () => {
  ...
  describe('scheduled (Triage-less) attempts: messageId null', () => {
    it('per_message Limits are not applicable — no denial, no counter row', async () => {
      const { userId } = await seedUserAndMessage(db)
      // mail_sender.send_message has per_message max 1; a scheduled send must
      // sail past it repeatedly with no per-message counter created.
      for (let i = 0; i < 3; i++) {
        const r = await checkAndConsumeLimits(
          db,
          {
      ...
      const messageCounters = await db.selectFrom('limit_counters_message').selectAll().execute()
      expect(messageCounters).toEqual([])
    })

    it('per_window Limits still bind: the send_message daily cap denies at 5', async () => {
      const { userId } = await seedUserAndMessage(db)
      const results: boolean[] = []
      for (let i = 0; i < 6; i++) {
        const r = await checkAndConsumeLimits(
