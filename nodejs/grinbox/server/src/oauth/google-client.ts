...
/** The PKCE challenge-method enum `generateAuthUrl` takes, named off the client. */
type CodeChallengeMethod = NonNullable<
...
export interface RefreshResult {
  ...
  readonly expiresInSeconds: number
  /**
   * A replacement refresh token, where the provider rotated one in. Storing it
   * is what keeps the next renewal working (d-v5fyd7xd); absent means the
   * durable credential stands.
   */
  readonly refreshToken?: string
}
