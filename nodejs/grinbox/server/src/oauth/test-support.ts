...
export function makeFakeGoogleClient(options: FakeGoogleOptions = {}): FakeGoogleClient {
  ...
  return {
    ...
    async exchangeCode(args): Promise<TokenExchangeResult> {
      ...
      if (options.omitRefreshToken) {
        return { ...base, refreshToken: undefined }
      }
      return base
    },
    async fetchProfileEmail(accessToken): Promise<string> {
      calls.profile.push(accessToken)
      return options.email ?? 'mailbox@example.com'
    },
    async refreshAccessToken(refreshToken): Promise<RefreshResult> {
      calls.refresh.push(refreshToken)
      if (options.refresh === 'invalid_grant') {
        throw new InvalidGrantError()
      }
      return options.refresh ?? { accessToken: 'access-new', expiresInSeconds: 3600 }
    },
  }
}
