...
function run(partial: Partial<ClassifyRun> & { operatorId: number }): ClassifyRun {
  return {
    ...
    extractedOutputKeys: partial.extractedOutputKeys ?? [],
...
describe('classifyInputs', () => {
  ...
  // An extracted output may be absent after a run that succeeded (d-04s2n18o):
  // a value that fails normalization drops its tag. Its consumer therefore runs
  ...
  it('runs the consumer of an absent extracted output', () => {
    const owner = run({
      operatorId: 1,
      outputKeys: ['amount'],
      extractedOutputKeys: ['amount'],
      status: 'completed',
    })
    const consumer = run({ operatorId: 2, inputKeys: ['amount'] })
