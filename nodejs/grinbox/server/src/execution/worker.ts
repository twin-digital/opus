 ...
 *     (`body-fetch.ts`), then builds the `MessageView`.
...
import { ensureMessageBody } from './body-fetch.js'
...
export async function runWorker(
  ...
  try {
    ...
    // Lazy body fetch: when this run's Operator consumes the Message body and
    // it isn't cached yet, fetch + persist it through the metered client (so
    // the fetch is Limit-checked and evented against this run). A failed /
    // skipped fetch leaves the body fields NULL and the run proceeds — a
    // `{{body}}` placeholder degrades to the empty string.
    const body = await ensureMessageBody(db, run, messageRow, makeResourceClient)
    const message = messageViewFromRow({
      ...messageRow,
      ...body,
...
interface WorkerContext {
  /**
   * The raw `messages` row. The worker builds the `MessageView` from it AFTER
   * the lazy body fetch, which may add the body fields (see
   * {@link ensureMessageBody}).
   */
  readonly messageRow: MessageRow
...
/**
 * A SELECT-all `messages` row: the resolved {@link import('kysely').Selectable}
 * form of {@link MessagesTable} (`id` is `number`, not `Generated`).
 */
type MessageRow = Omit<MessagesTable, 'id'> & { readonly id: number }
...
async function loadContext(db: DB, run: WorkerRunRow): Promise<WorkerContext> {
  ...
  return {
    messageRow: messageRow as unknown as MessageRow,
    tags,
