...
async function seedTriage(db: DB, args: SeedTriageArgs): Promise<void> {
  ...
  if (applyCompleted) {
    await event(ops.apply, 'resource_op_succeeded', { resource: 'mailbox', operation: 'apply_category' }, startedAt + 4)
  }
 ...
 * limits, and a couple of per-message counters for the apply_category per-window /
...
async function seedLimitUsage(db: DB, userId: number, now: number): Promise<void> {
  ...
  const applyCategoryWindow = find('mailbox', 'apply_category', 'per_window')
  ...
  if (applyCategoryWindow) {
    windowRows.push({
      limit_id: applyCategoryWindow,
