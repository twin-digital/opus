 ...
 *  3. Validate the *post-state* via {@link validatePipeline}. Create/edit
 *     additionally run the save-time-only checks on the changed Operator —
 *     template placeholder names, `when`-gate producer/enum membership, digest
 *     section validity ({@link saveTimeValidationErrors}).
...
import { type OperatorTypeKey, operatorTypeKeySchema } from '@grinbox/shared'
...
/**
 * Runs the save-time-only checks (template placeholders, `when`-gate
 * producer/enum membership, digest section validity) for the Operator being
 * created/edited, against the post-change enabled set. Applied to that
 * Operator only (not the whole set, and not on enable/disable/delete): the
 * goal is to stop new mistakes at the door without letting a pre-existing
 * violation elsewhere in the Pipeline block unrelated saves.
 */
function assertSaveTimeChecks(op: OperatorForValidation, postState: readonly OperatorForValidation[]): void {
  const errors = saveTimeValidationErrors(op, postState)
  if (errors.length > 0) {
...
/**
 * Resolves the `type_code_version` to capture for a `type_key` from the
 * behavioral registry's deployed `code_version`. The `'1'` fallback (the
 * registry.ts starting-version convention) covers a type the registry doesn't
...
export async function createOperator(db: Kysely<Database>, input: CreateOperatorInput): Promise<number> {
  ...
  return withPipelineEditLock(db, async (tx) => {
    ...
    assertValid(enabled)
    assertSaveTimeChecks({ operator_id: 0, type_key: typeKey, config_json: input.configJson }, enabled)
