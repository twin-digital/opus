...
/**
 * Unknown `{{...}}` placeholder names are rejected at create/edit — the
 * renderer swallows them to `""` at run time, so a typo like `{{Body}}` would
 * otherwise silently degrade the rendered output. Enable/disable stay
 * unaffected: a stored config with a stray placeholder keeps operating as it
 * always has.
 */
describe('Operator save — unknown template placeholders', () => {
  let db: Kysely<Database>
  let seed: SeedResult
  ...
  it('create rejects a template with an unknown placeholder', async () => {
    await expect(
      createOperator(db, {
        pipelineId: seed.pipelineId,
        name: 'llm',
        typeKey: 'llm_tagger',
        configJson: llmConfig('classify {{Body}}'),
        enabled: true,
        actorUserId: null,
      }),
  ...
  it('create rejects even when the Operator is saved disabled', async () => {
    await expect(
      createOperator(db, {
        pipelineId: seed.pipelineId,
        name: 'llm',
        typeKey: 'llm_tagger',
        configJson: llmConfig('classify {{email}}'),
        enabled: false,
        actorUserId: null,
      }),
    ).rejects.toThrow(PipelineValidationError)
  ...
  it('edit rejects introducing an unknown placeholder', async () => {
    const opId = await createOperator(db, {
      pipelineId: seed.pipelineId,
      name: 'llm',
      typeKey: 'llm_tagger',
      configJson: llmConfig('classify {{subject}} {{body}}'),
      enabled: true,
      actorUserId: null,
    })
    await expect(
      editOperator(db, {
        operatorId: opId,
        configJson: llmConfig('classify {{Subject}}'),
        actorUserId: null,
      }),
    ).rejects.toThrow(PipelineValidationError)
  })

  it('enable/disable of a stored config with a stray placeholder still works', async () => {
    // Simulate a config saved before this validation existed: write it
    // directly, bypassing createOperator.
    const inserted = await db
      ...
      .values({
        ...
        type_key: 'llm_tagger',
        type_code_version: '1',
        config_json: llmConfig('classify {{Body}}'),
        enabled: 0,
        created_at: 1000,
        updated_at: 1000,
        deleted_at: null,
    ...
    await setOperatorEnabled(db, inserted.id, true, null)
    await setOperatorEnabled(db, inserted.id, false, null)
    const op = await db
      .selectFrom('operators')
      .select('enabled')
