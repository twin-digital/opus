...
import { ruleBasedTaggerConfigSchema } from '@grinbox/shared'
...
export function createOperatorsRoutes(deps: ApiDeps) {
  return new Hono().post('/preview', zValidator('json', previewBody), async (c) => {
    ...
    try {
      ...
      // Same `{ error: { code, message } }` envelope as every other route
      // (write-errors.ts), so the web's shared error parser reads it.
