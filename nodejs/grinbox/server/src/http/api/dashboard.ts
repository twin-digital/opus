...
export function createDashboardRoutes(deps: ApiDeps) {
  return new Hono().get('/', async (c) => {
    ...
    // Extracted Tag keys are excluded from top-tags: their values have
    // unbounded cardinality, so per-value counts are meaningless. There is no
    // "extracted" column on `tags` — extractedness is a property of the
    // producing Operator's declared output, derived here from the enabled
    // Operators' current configs (`value_type` outputs).
    const extractedKeys = await extractedTagKeys(deps.db)
    ...
    const [
      ...
      deps.db
        ...
        .$if(extractedKeys.length > 0, (qb) => qb.where('tg.key', 'not in', extractedKeys))
...
/**
 * The Tag keys declared as *extracted* (`value_type`) outputs by any enabled,
 * non-deleted LLM Tagger, across all Pipelines. Derived on demand from
 * `operators.config_json` — extractedness lives on the producer's declaration,
 * not on the `tags` rows. A config that no longer parses contributes nothing.
 */
async function extractedTagKeys(db: DB): Promise<string[]> {
  const rows = await db
    .selectFrom('operators')
    .select(['config_json'])
    .where('type_key', '=', 'llm_tagger')
    .where('enabled', '=', 1)
    .where('deleted_at', 'is', null)
    .execute()
  ...
  for (const row of rows) {
    ...
    const parsed = operatorConfigSchemas.llm_tagger.safeParse(parsedJson)
    if (!parsed.success) {
      continue
    }
    for (const output of parsed.data.outputs) {
      if ('value_type' in output) {
        keys.add(output.tag_key)
      }
    }
  }
  return [...keys]
