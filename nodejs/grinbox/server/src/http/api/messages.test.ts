...
describe('an outcome resolves to the configuration that produced it', () => {
  ...
  // d-nr71oscu snapshots the configuration a run was enqueued with, and
  ...
  it.skip("returns each run's configuration snapshot on message detail", async () => {
    ...
    const acctId = await insertAccount(db, userId, { activePipelineId: pid })
    const m = await insertMessage(db, acctId, { backendMessageId: 'm1', receivedAt: 1000 })
    await insertTriage(db, {
      messageId: m,
      pipelineId: pid,
      operatorId: opId,
      startedAt: 1100,
      makeCurrent: true,
      opConfigJson: JSON.stringify({ output_tag_key: 'urgency', marker: 'as-enqueued' }),
      tags: [{ key: 'urgency', value: 'high' }],
    })
    // The operator has since been edited; the run must not resolve to this.
    await db
      .updateTable('operators')
      .set({ config_json: JSON.stringify({ output_tag_key: 'urgency', marker: 'edited-since' }) })
      .where('id', '=', opId)
      .execute()
  ...
  it.skip("shows one triage's tags, not a union across pipelines", async () => {
    ...
    const opA = await insertOperator(db, pidA, {
      name: 'a',
      typeKey: 'rule_based_tagger',
      configJson: ruleTaggerConfig('urgency', ['high', 'low']),
    })
    const opB = await insertOperator(db, pidB, {
      name: 'b',
      typeKey: 'rule_based_tagger',
      configJson: ruleTaggerConfig('topic', ['bill', 'other']),
    })
    const acctId = await insertAccount(db, userId, { activePipelineId: pidA })
    const m = await insertMessage(db, acctId, { backendMessageId: 'm1', receivedAt: 1000 })
    await insertTriage(db, {
      messageId: m,
      pipelineId: pidA,
    ...
    await insertTriage(db, {
      ...
      startedAt: 1200,
      makeCurrent: true,
      tags: [{ key: 'topic', value: 'bill' }],
    })

    const app = createApiRoutes({ db, now: fixedNow })
    const body = (await (await app.request('/api/messages')).json()) as MessageListResponse
    const row = body.messages.find((x) => x.id === m)
    const pipelines = new Set((row?.current_tags ?? []).map((t) => t.pipeline_id))
