...
describe('Limit write routes', () => {
  ...
  // d-qv5l66ya: the caps grinbox seeds are the backstop and cannot be removed or
  ...
  it.skip('refuses to delete a seeded cap', async () => {
    ...
    const res = await app.request(jsonReq(`/api/limits/${seeded.id}`, 'DELETE'))
    expect(res.status).toBe(409)
    const still = await db.selectFrom('limits').select('id').where('id', '=', seeded.id).executeTakeFirst()
  ...
  it.skip('refuses to loosen a seeded cap, and allows tightening it', async () => {
    ...
    const seeded = await db
      .selectFrom('limits')
      .select(['id', 'max_count', 'window_seconds'])
      .where('scope', '=', 'per_window')
      .orderBy('id', 'asc')
      .executeTakeFirstOrThrow()
    const app = appFor(db)

    const loosen = await app.request(
      jsonReq(`/api/limits/${seeded.id}`, 'PATCH', {
        max_count: seeded.max_count + 1,
        window_seconds: seeded.window_seconds,
      }),
    )
    expect(loosen.status).toBe(409)
    const unchanged = await db.selectFrom('limits').selectAll().where('id', '=', seeded.id).executeTakeFirstOrThrow()
    expect(unchanged.max_count).toBe(seeded.max_count)
  })

  // A user cap layers over the seeded one rather than replacing it: both bind,
  ...
  it.skip('accepts a user cap on an operation a seeded cap already covers', async () => {
    ...
    const both = await db
      .selectFrom('limits')
      .select('id')
      .where('resource', '=', seeded.resource)
      .where('operation', '=', seeded.operation)
      .where('scope', '=', 'per_window')
      .execute()
    expect(both).toHaveLength(2)

    // The user's own cap comes back off freely, leaving the seeded one standing.
    const { id } = (await (
      await app.request(
        jsonReq('/api/limits', 'POST', {
          resource: 'pushover_api',
    ...
    expect((await app.request(jsonReq(`/api/limits/${id}`, 'DELETE'))).status).toBe(200)
  })
