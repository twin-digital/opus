 ...
 *     failures, and
 *   - `digest_runs`: failed runs (as `digest_run_failed`) plus the
 *     limited/failed Resource-op events accumulated in their `events_json`
 *     (digest runs happen outside any Triage, so their events live on the run
 *     row rather than in `triage_events`).
...
export interface ActivityEntry {
  ...
  readonly source: 'triage_event' | 'operator_run' | 'digest_run'
  readonly severity: ActivitySeverity
  readonly event_type: string
  readonly resource: string | null
  readonly operation: string | null
  /** `null` for digest-run entries — a digest run belongs to no Triage. */
  readonly triage_id: number | null
...
export function createActivityRoutes(deps: ApiDeps) {
  return new Hono().get('/', zValidator('query', query), async (c) => {
    ...
    // Source 3: digest runs — failed runs plus the limited/failed Resource-op
    // events accumulated on each run row (`events_json`). Events carry no
    // per-event timestamp; the run's `finished_at` stands in for all of them.
    const digestRows = await deps.db
      .selectFrom('digest_runs')
      .select(['id', 'operator_id', 'status', 'error_summary', 'events_json', 'started_at', 'finished_at'])
      .where((eb) => eb.or([eb('status', '=', 'failed'), eb('events_json', 'is not', null)]))
      .execute()
    for (const r of digestRows) {
      const recordedAt = r.finished_at ?? r.started_at
      if (r.status === 'failed') {
        entries.push({
          source: 'digest_run',
          severity: 'error',
          event_type: 'digest_run_failed',
          resource: null,
          operation: null,
          triage_id: null,
          operator_id: r.operator_id,
      ...
      for (const ev of parseDigestEvents(r.events_json)) {
        if (ev.event_type !== 'resource_op_limited' && ev.event_type !== 'resource_op_failed') {
          continue
        }
        const details = ev.details
        entries.push({
          source: 'digest_run',
          severity: ev.event_type === 'resource_op_limited' ? 'warning' : 'error',
    ...
    // Filter + sort most-recent-first across the sources, then paginate.
...
/** Parse a digest run's `events_json` (array of `{ event_type, details }`). */
function parseDigestEvents(json: string | null): { event_type: string; details: Record<string, unknown> }[] {
  if (!json) {
    return []
  }
  try {
