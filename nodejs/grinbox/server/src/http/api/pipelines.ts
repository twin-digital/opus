...
export interface TagKeyRegistryEntry {
  ...
  /** Enum values for closed-vocabulary outputs; `null` for extracted outputs. */
  readonly value_enum: readonly string[] | null
  /** The extracted value type for extracted outputs; `null` for enum outputs. */
  readonly value_type: 'string' | 'money' | 'date' | null
}
      ...
      for (const op of operatorRows) {
        ...
        for (const out of contract.outputs) {
          tagKeyRegistry.push({
            ...
            value_enum: 'valueEnum' in out ? out.valueEnum : null,
            value_type: 'valueType' in out ? out.valueType : null,
          })
        }
      }
