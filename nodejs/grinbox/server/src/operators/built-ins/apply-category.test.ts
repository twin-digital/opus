...
function args(canned: ResourceOpResult<unknown> | undefined, tags: Record<string, string> = {}) {
  const fake = createFakeResourceClients(canned ? { canned: { 'mailbox.apply_category': canned } } : {})
  return {
    fake,
    runArgs: {
      message: message(),
...
function categoryApplied(): ResourceOpResult<unknown> {
...
describe('apply-category run', () => {
  it('applies the templated category (renders Tags, passes backend id)', async () => {
    const { fake, runArgs } = args(categoryApplied(), { topic: 'Finance' })
    const result = await runOperator(snapshot(), runArgs)

    expect(result.tags).toEqual([])
    const call = fake.calls.find((c) => c.operation === 'apply_category')
    expect(call).toBeDefined()
    const sent = call?.args as { backendMessageId: string; category: string }
    expect(sent.backendMessageId).toBe('gmail-abc')
    expect(sent.category).toBe('Grinbox/Finance')
  })
  ...
  it('fires when the optional `when` gate matches', async () => {
    const { fake, runArgs } = args(categoryApplied(), { topic: 'Travel' })
    await runOperator(snapshot({ when: { tag_key: 'topic', equals: ['Travel'] } }), runArgs)
    expect(fake.calls.find((c) => c.operation === 'apply_category')).toBeDefined()
  })
