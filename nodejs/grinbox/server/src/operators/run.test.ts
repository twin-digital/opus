import { type RuleBasedTaggerConfig, contractFromConfig, ruleBasedTaggerConfigSchema } from '@grinbox/shared'
...
describe('runOperator with O1 (Rule-based Tagger)', () => {
  ...
  it('a when gate that does not match emits nothing — not even the fallback', async () => {
    const gated: RuleBasedTaggerConfig = {
      ...urgencyConfig,
      when: { tag_key: 'digest_category', equals: ['bill'] },
    }
    const { runArgs } = args(message(), { digest_category: 'none' })
    const result = await runOperator(snapshot(gated), runArgs)
    expect(result.tags).toEqual([])
  })

  it('a when gate that matches leaves evaluation (incl. fallback) unchanged', async () => {
    const gated: RuleBasedTaggerConfig = {
      ...urgencyConfig,
      when: { tag_key: 'digest_category', equals: ['bill'] },
    }
    const { runArgs } = args(message({ subject: 'nothing special', from: 'x@y.z' }), {
      digest_category: 'bill',
    })
    const result = await runOperator(snapshot(gated), runArgs)
