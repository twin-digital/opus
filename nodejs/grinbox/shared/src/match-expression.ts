...
export function extractTagRefs(expression: string): string[] {
  return extractFieldRefsOfKind(expression, 'tag')
}

/**
 * Parses a `match` expression and returns the distinct bare Message fields it
 * references (`from`, `subject`, `body`, …). Used to detect a Rule that reads
 * the Message body, which makes its Rule-based Tagger a body-consuming
 * Operator for the lazy body fetch. Same parse semantics as
 * {@link extractTagRefs}: throws {@link MatchExpressionError} on a malformed
 * expression; order of first appearance, duplicates removed.
 */
export function extractMessageFieldRefs(expression: string): string[] {
  return extractFieldRefsOfKind(expression, 'message')
}

/** Shared AST walk for {@link extractTagRefs} / {@link extractMessageFieldRefs}. */
function extractFieldRefsOfKind(expression: string, kind: FieldRef['kind']): string[] {
  const ast = new Parser(tokenize(expression)).parse()
  const keys: string[] = []
  ...
  const walk = (node: Node): void => {
    switch (node.kind) {
      ...
      case 'not':
        ...
        return
      case 'comparison':
        if (node.field.kind === kind && !seen.has(node.field.name)) {
