...
/**
 * `extractMessageFieldRefs` mirrors `extractTagRefs` for the bare Message
 * fields — the lazy body fetch uses it to spot a Rule that reads `body`.
 */
describe('extractMessageFieldRefs', () => {
  it('returns the bare Message fields referenced, in first-seen order', () => {
    expect(extractMessageFieldRefs('subject contains "x" or body contains "y"')).toEqual(['subject', 'body'])
  })

  it('dedupes repeated field refs', () => {
    expect(extractMessageFieldRefs('body contains "a" and body contains "b"')).toEqual(['body'])
  })

  it('ignores header./tag./thread. refs and string operands', () => {
    expect(
      extractMessageFieldRefs(
        'header."list-id" contains "news" and tag.kind == "body" ' + 'and thread.is_reply == "no"',
      ),
    ).toEqual([])
  })

  it('finds fields nested under not/parentheses', () => {
    expect(extractMessageFieldRefs('not (from == "a" or body matches /invoice/i)')).toEqual(['from', 'body'])
  })

  it('throws MatchExpressionError on a malformed expression', () => {
    expect(() => extractMessageFieldRefs('(((')).toThrow(MatchExpressionError)
  })
})

// r-ut5mdtpx — what a user configures is never executed as code. The rule
// language is read and evaluated by compileMatch (d-oq267pmh) and the template
// grammar by its own reader (d-cnjkypbb); neither hands anything the user wrote
// to a language runtime. Asserted over the sources so a later shortcut through
// eval, new Function, or node:vm fails here.

describe('nothing a user configures reaches a language runtime', () => {
  it('uses no dynamic code evaluation anywhere in the package', async () => {
    ...
    for (const file of files) {
      ...
      if (/\beval\s*\(|new Function\s*\(|node:vm\b/.test(source)) {
        offenders.push(file)
      }
    }
