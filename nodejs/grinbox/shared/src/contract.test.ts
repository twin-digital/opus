...
describe('contractFromConfig — inputs', () => {
  ...
  describe('archive', () => {
    it('declares the when.tag_key as its input', () => {
      const config: OperatorConfigFor<'archive'> = {
        when: { tag_key: 'disposition', equals: ['archive'] },
      }
      expect(contractFromConfig('archive', config).inputs).toEqual(['disposition'])
    })

    it('declares no inputs without `when`', () => {
      const config: OperatorConfigFor<'archive'> = {}
      expect(contractFromConfig('archive', config).inputs).toEqual([])
    })
  })
  ...
  describe('rule_based_tagger', () => {
    ...
    it('tolerates an unparseable Rule `match`: skips its refs, never throws', () => {
      const config = {
        ...
        fallback: { output: 'b' },
      } as OperatorConfigFor<'rule_based_tagger'>
