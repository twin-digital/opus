...
import { type ActivityEntry, type ActivitySeverity, useActivity } from '@/lib/activity'
import { relativeTime } from '@/lib/format'
import { type ActivitySearch, filtersFromSearch, hasActiveFilters } from './search'
...
function pruneSearch(merged: ActivitySearch, touched: Partial<ActivitySearch>): ActivitySearch {
  // A key the caller touched back to `undefined` is dropped rather than carried,
  // so it leaves the URL instead of appearing as an empty query parameter.
  const dropped = new Set(Object.keys(touched).filter((key) => (merged as Record<string, unknown>)[key] === undefined))
  return Object.fromEntries(Object.entries(merged).filter(([key]) => !dropped.has(key)))
}
