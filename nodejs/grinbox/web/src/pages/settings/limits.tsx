import { RESOURCE_OPERATIONS, type Resource, limitDefinitionSchema } from '@grinbox/shared'
...
import { isSeeded, type LimitRowEntry, useCreateLimit, useDeleteLimit, useEditLimit, useLimits } from '@/lib/limits'
...
function LimitsTable({ limits }: { limits: LimitRowEntry[] }) {
  return (
...
/**
 * One cap. A seeded cap — grinbox's own backstop — carries no edit and no
 * delete control: it cannot be removed or loosened by anyone (d-qv5l66ya), and
 * r-zmn2p7lf is read by looking for exactly such an affordance and not finding
 * one. The user's own caps layer over the seeded ones and stay fully editable.
 */
function LimitRow({ limit }: { limit: LimitRowEntry }) {
  const seeded = isSeeded(limit)
    ...
    <TableRow>
      ...
      <TableCell>
        <div className='flex items-center justify-end gap-1'>
          {seeded ?
            <span className='text-xs text-muted-foreground'>Grinbox's own cap — add your own to tighten it</span>
          : <>
...
function usageLabel(limit: LimitRowEntry): string {
...
function EditLimitDialog({ limit }: { limit: LimitRowEntry }) {
...
function DeleteLimitDialog({ limit }: { limit: LimitRowEntry }) {
