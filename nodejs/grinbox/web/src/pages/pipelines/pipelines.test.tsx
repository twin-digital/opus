import type { OperatorDetail, PipelineDetail, PipelineSummary } from '@grinbox/server'
...
const usePipeline = vi.fn()
const useCreatePipeline = vi.fn()
const useUpdatePipeline = vi.fn()
const useDeletePipeline = vi.fn()
const useCreateOperator = vi.fn()
const useUpdateOperator = vi.fn()
const useSetOperatorEnabled = vi.fn()
const useDeleteOperator = vi.fn()
const useCredentials = vi.fn()
const useOperatorPreview = vi.fn()
...
import { PipelineDetailPage } from './detail'
import { OperatorEditor } from './editors/operator-editor'
import { PipelinesListPage } from './list'
...
describe('OperatorEditor', () => {
  function renderEditor(props: Partial<React.ComponentProps<typeof OperatorEditor>> = {}) {
    ...
    // The editor tree includes hooks that query through react-query (the LLM /
    // Digest model pickers), so it needs a provider like the page renders do.
    renderPage(
      <OperatorEditor
        open
        onOpenChange={vi.fn()}
  ...
  it('surfaces a pipeline_validation_failed error and keeps the editor open', async () => {
    ...
    const onSave = vi
      .fn()
      .mockRejectedValue(
        new ApiError('pipeline_validation_failed', 'Pipeline validation failed.', [
