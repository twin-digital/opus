...
import { type OperatorPreviewResponse, useOperatorPreview } from '@/lib/pipelines'
import { displayFrom } from '@/pages/inbox/list'
import { type ActionWhenDraft, ActionWhenField } from './action-editors'
import { type StructuredMatch, blankStructuredMatch, composeMatch, parseStructuredMatch } from './match-builder'
...
function PreviewBody({
  ...
  // The pending and error branches above are what narrow `data` to the result.
        ...
        {changed.map((r) => (
          <li key={r.message_id} className='rounded-md border border-border bg-card p-2 text-xs'>
            ...
            <div className='truncate text-muted-foreground' title={r.subject ?? undefined}>
              {r.subject === null || r.subject === '' ? '(no subject)' : r.subject}
            </div>
