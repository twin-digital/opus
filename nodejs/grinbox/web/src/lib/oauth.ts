...
/**
 * Whether an arbitrary `postMessage` payload is one of ours. The data crosses a
 * window boundary, so it arrives as `unknown` and is narrowed rather than cast:
 * anything on that channel can post, and the origin check alone does not make
 * the body the shape it claims.
 */
function isOAuthMessage(data: unknown): data is OAuthMessage {
  return (
    typeof data === 'object' &&
    data !== null &&
    (data as { source?: unknown }).source === 'grinbox-oauth' &&
...
const CALLBACK_ORIGIN_OVERRIDE = import.meta.env.VITE_OAUTH_CALLBACK_ORIGIN
