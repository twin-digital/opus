...
import { messageKey, toQueryParams, useReplayMessage } from './messages'
...
const replayPost = vi.fn(async () => ({ ok: true, json: async () => ({}) }))
vi.mock('./api', () => ({
  apiBase: '',
  api: {
    api: {
...
describe('toApiError', () => {
  ...
  it('falls back generically on a body that is not the declared envelope', async () => {
    // The envelope is declared once in @grinbox/shared and the daemon composes
    // every refusal from it, so a body that does not parse against it is not a
    // refusal this client can read a code out of.
...
describe('errorMessage', () => {
  it('returns an ApiError message', () => {
    expect(errorMessage(new ApiError('x', 'boom'))).toBe('boom')
  })
