 ...
 * server; error responses surface via the shared {@link toApiError}. The Inbox
 * list query closes over the active filter set; the detail query carries the
 * full Triage history; Replay is a mutation that invalidates the message
 * detail + list.
 */
 ...
 *
 * `op_config_json` is the configuration snapshot the run executed against
 * (d-nr71oscu) — what makes a historical outcome resolve to the configuration
 * that produced it rather than the current one (r-k6gh82fx).
 */
