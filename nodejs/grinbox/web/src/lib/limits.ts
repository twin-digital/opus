import type { LimitEntry } from '@grinbox/server'
import type { LimitDefinition } from '@grinbox/shared'
...
/**
 * A limit as the settings table renders it.
 *
 * `origin` says who put the cap there: `seeded` caps are grinbox's own backstop
 * and can be neither removed nor loosened by anyone, and the user's own caps
 * layer over them (d-qv5l66ya). The interface offers no way to do either to a
 * seeded row, which is half of what r-zmn2p7lf promises — the server refuses it
 * too.
 */
export type LimitRowEntry = LimitEntry

/** Whether this cap is one grinbox seeded, and so is not the user's to change. */
export function isSeeded(limit: LimitRowEntry): boolean {
  return limit.origin === 'seeded'
}

export function useLimits() {
  return useQuery({
    queryKey: limitsKey,
    queryFn: async (): Promise<LimitRowEntry[]> => {
