...
`@thrashplay/launchpad-sim` (browser-based hardware simulator). opus-scripts gains a vite builder:
`build` dispatches to `vite build` for packages with a `vite.config.*`, ahead of the tsc fallback.
